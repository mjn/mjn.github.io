...
module.exports = function (eleventyConfig) {
  ...
  eleventyConfig.addCollection("post", function (collectionApi) {
    return collectionApi
      .getFilteredByGlob("src/data/blog/**/*.md")
      .sort((a, b) => b.date - a.date)
  })

  eleventyConfig.addCollection("experience", function (collectionApi) {
    return collectionApi
      .getFilteredByGlob("src/data/experience/*.md")
      .sort((a, b) => {
        const dateA = a.data.startDate ? new Date(a.data.startDate) : new Date()
        const dateB = b.data.startDate ? new Date(b.data.startDate) : new Date()
        return dateB - dateA
      })
  })
          ...
          for (const img of images) {
            ...
            if (fs.existsSync(imagePath)) {
              try {
                ...
                const pictureHtml = Image.generateHTML(
                  metadata,
                  imageAttributes
                )
